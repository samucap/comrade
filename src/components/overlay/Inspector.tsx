...
import { Trash2, Move, Rotate3D, Scaling, Box, Layers, ChevronDown, ChevronRight } from 'lucide-react';
...
export function Inspector() {
    ...
    const [isSceneOpen, setIsSceneOpen] = useState(true);
    ...
    return (
        <aside className="absolute top-0 right-0 h-full w-80 p-4 pointer-events-auto flex flex-col gap-4 overflow-y-auto">
            {/* Scene List Panel */}
            <Panel className="w-full flex flex-col min-h-0 overflow-hidden shrink-0">
                <button
                    onClick={() => setIsSceneOpen(!isSceneOpen)}
                    className="flex items-center gap-2 p-4 border-b border-zinc-800 hover:bg-zinc-900/50 transition-colors w-full text-left"
                >
                    {isSceneOpen ? <ChevronDown className="w-4 h-4 text-zinc-500" /> : <ChevronRight className="w-4 h-4 text-zinc-500" />}
            ...
            {/* Properties Panel */}
            <Panel className="w-full flex flex-col shrink-0">
                <button
                    onClick={() => setIsPropsOpen(!isPropsOpen)}
                    className="flex items-center gap-2 p-4 border-b border-zinc-800 hover:bg-zinc-900/50 transition-colors w-full text-left"
                >
                    {isPropsOpen ? <ChevronDown className="w-4 h-4 text-zinc-500" /> : <ChevronRight className="w-4 h-4 text-zinc-500" />}
                                ...
                                <div className="flex justify-between items-center">
                                    ...
                                    <Tooltip content="Delete Object">
                                        <Button
                                            ...
                                            className="p-2 h-auto text-red-500 hover:text-red-500 hover:bg-red-500/10 border-red-500/20 self-end"
                                        >
